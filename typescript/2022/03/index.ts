...
const determineSharedItemType = (rucksack: string): string | null => {
    const rucksackCompartmentSize: number = rucksack.length / 2;
    const compartments: string[] = [
        rucksack.slice(0, rucksackCompartmentSize),
        rucksack.slice(rucksackCompartmentSize, rucksack.length),
    ];
...
// const sharedItemTypesPerRucksack = rucksacks.map(rucksack => determineSharedItemType(rucksack))
const allRucksackItemPriorities: number[] = rucksacks.map((rucksack) => {
    const sharedItemType = determineSharedItemType(rucksack);
    if (sharedItemType === null) {
        console.error(`${rucksack} - Unable to determine sharedItemType`);
        return 0;
    }
    const itemValue = determineItemValue(sharedItemType);
    return itemValue;
});

const answer = allRucksackItemPriorities.reduce(
